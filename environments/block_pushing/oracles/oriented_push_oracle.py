...
class OrientedPushOracle(py_policy.PyPolicy):
  ...
  def _get_action_for_block_target(
      self, time_step, block="block", target="target"):
...
class OrientedPushNormalizedOracle(py_policy.PyPolicy):
  ...
  def __init__(self, env):
    super(OrientedPushNormalizedOracle, self).__init__(env.time_step_spec(),
                                                       env.action_spec())
  ...
  def _action(self, time_step,
    ...
    return policy_step.PolicyStep(action=self._env.calc_normalized_action(
        step.action))
