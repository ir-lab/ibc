...
lgg.getLogger('tensorflow').setLevel(lgg.FATAL)
...
def train_eval(
  ...
  if train_cluster:
    ...
    import time
    # prevent gpu files from being created all at once
    time.sleep(np.random.rand()*20)
    gpu_dir = Path(root_dir).parent / 'gpus'
    mapped_gpu = False
    gpu_to_use = ''
    while not mapped_gpu:
      gpu_devs = set([x.name for x in tf.config.list_logical_devices('GPU')])
      
      if len(gpu_devs) != 4:
        print(f"WARNING: {len(gpu_devs)} GPUs detected!")
      
      used_gpus = set()
      for gpu_file in list(gpu_dir.glob('*.gpu')):
          with open(gpu_file.as_posix(), 'r') as f:
            gpu_name = f.readline()
          if gpu_name in gpu_devs:
            # print(f"Skipping {gpu_name}...")
            used_gpus.add(gpu_name)

      # print("All gpus: ")
      # print(gpu_devs)
      avail_gpus = list(gpu_devs - used_gpus)
      # print("Avail gpus: ")
      # print(avail_gpus)
      if len(avail_gpus) >= 1:
        gpu_to_use = avail_gpus[0]
        print(f"Using {gpu_to_use}")
        cur_gpu_file = gpu_dir / f"{Path(root_dir).name}.gpu"
        with open(cur_gpu_file.as_posix(), 'w') as f:
          f.write(gpu_to_use)
        mapped_gpu = True
      else:
    ...
    strategy = tf.distribute.OneDeviceStrategy(gpu_to_use)
  ...
  try:
    ...
    if add_time:
      ...
      # hashids = Hashids()
      # hash = hashids.encode(calendar.timegm(time.gmtime()))
      # import uuid
      # current_time = str(uuid.uuid4())
    ...
    for run in range(runs):
      ...
      if (not train_cluster) and (not strategy):
      ...
      while train_step.numpy() < num_iterations:
        ...
        if not skip_eval and train_step.numpy() % eval_interval == 0:
            ...
            proto_name = os.path.join(proto_path, f"algo=ibc,run={run}-{train_step.numpy()}")
  ...
  if train_cluster:
    print(f"Removing {cur_gpu_file.as_posix()}...")
    cur_gpu_file.unlink()
...
def main(_):
  ...
  # cluster_resolver = tf.distribute.cluster_resolver.SlurmClusterResolver(gpus_per_task=1, tasks_per_node=4, gpus_per_node=4)
